...
RETURNING id, account, display_name, created_at;
...
SELECT
    ...
    (SELECT id FROM users WHERE account = 'admin' AND is_deleted = false LIMIT 1)
WHERE NOT EXISTS (
    SELECT 1 FROM roles
...
SET is_deleted = false,
    deleted_at = NULL,
    deleted_by = NULL
WHERE user_id = (SELECT id FROM users WHERE account = 'admin' AND is_deleted = false LIMIT 1)
...
WHERE u.account = 'admin'
  AND u.is_deleted = false
GROUP BY u.id, u.account, u.display_name, u.created_at, r.role_name;
