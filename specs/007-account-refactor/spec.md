...
- **密碼重設的頻率限制**: 是否需要限制密碼修改/重設的頻率,以防止濫用? → **v1 不考慮**,依賴現有系統的一般性防護機制
- **審計日誌的保留期限**: 密碼重設的審計日誌應該保留多久? → 遵循系統既有的審計日誌保留政策
- **權限變更的即時生效**: 當用戶的 account.* 權限被修改後,是否立即生效還是需要重新登入? → 遵循現有權限系統的生效機制（通常需重新登入）
...
- **FR-009b**: 密碼修改成功後,系統必須清除該用戶的版本號快取(user_version:{userId}),確保所有舊 Token 在下次請求時被驗證為無效
...
- **FR-014b**: 密碼重設成功後,系統必須清除該用戶的版本號快取(user_version:{userId}),確保該用戶在所有設備上的舊 Token 在下次請求時被驗證為無效

#### 密碼驗證
- **FR-015**: 系統必須驗證新密碼符合密碼強度要求(參照現有系統的密碼驗證器規則：最小長度、字元類型要求等)
