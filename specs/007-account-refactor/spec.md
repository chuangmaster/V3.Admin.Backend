...
3. **Given** 舊的 API 客戶端仍使用 "username" 欄位發送請求, **When** 系統接收到請求, **Then** 系統能夠向後相容並正確處理(或明確返回欄位已棄用的訊息)
...
- **資料遷移期間的併發訪問**: 當資料庫正在執行 username 到 account 的遷移時,如果有 API 請求進來,系統如何處理?
- **密碼修改期間的會話管理**: 用戶修改密碼後,是否需要重新登入?現有的會話令牌是否立即失效?
- **管理員重設密碼的通知機制**: 當管理員重設用戶密碼後,系統是否應該通知該用戶?如何通知?
- **併發控制的競爭條件**: 如果兩個管理員同時嘗試重設同一用戶的密碼,且都使用相同的 version,系統如何處理?
- **向後相容性**: 舊的客戶端仍使用 "username" 欄位,系統是支援向後相容還是強制升級?
