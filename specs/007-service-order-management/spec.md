...
店員查詢、瀏覽、修改已建立的服務單,包含篩選條件搜尋、查看詳細資訊、修改服務單內容、更新狀態、管理附件。

**Why this priority**: 查詢與管理功能是日常營運的必要工具,需在建立功能完成後優先實現以支援後續的服務單維護。

**Independent Test**: 店員可以透過多種篩選條件（服務單類型、客戶名稱、日期範圍、狀態）搜尋服務單,查看詳細資訊、修改內容、更新狀態,並查看所有修改歷史。

**Acceptance Scenarios**:

1. **Given** 店員開啟服務單查詢介面，**When** 選擇篩選條件（服務單類型、客戶名稱、日期範圍、狀態）並點擊搜尋，**Then** 系統顯示符合條件的服務單列表（包含服務單編號、客戶名稱、商品資訊、金額、狀態），支援分頁瀏覽。

2. **Given** 店員點擊某筆服務單，**When** 進入詳細頁面，**Then** 系統顯示完整服務單資訊（客戶資料、商品項目、金額、狀態、附件列表、簽名記錄），店員可修改服務單內容並儲存變更。

3. **Given** 店員修改服務單資訊（如總金額），**When** 儲存變更，**Then** 系統檢查版本號（樂觀鎖）以避免並發更新衝突,更新成功後自動記錄修改歷史（變更欄位、舊值、新值、變更時間、變更者）。

4. **Given** 店員查看服務單詳細頁面，**When** 點擊「修改歷史」標籤，**Then** 系統顯示所有變更記錄（時間、變更者、變更欄位、舊值、新值），按時間倒序排列。

5. **Given** 店員查看服務單詳細頁面的附件區域，**When** 點擊下載身分證明文件或合約文件，**Then** 系統記錄附件查看日誌（用於個資稽核）並提供檔案下載連結。

6. **Given** 店員查看服務單詳細頁面，**When** 點擊狀態更新按鈕（「標記為已完成」、「標記為待處理」、「標記為已終止」），**Then** 系統驗證狀態轉換規則（終態不可逆）並更新服務單狀態。
...
- 多位店員同時修改同一筆服務單時,系統透過樂觀鎖（version 欄位）偵測並發更新衝突,最後儲存的店員收到錯誤訊息並要求重新載入最新資料。
...
- **FR-003**: 系統必須提供身分證明文件上傳功能,僅接受圖片格式（JPG、PNG）,檔案大小限制 10MB,每筆服務單至少需要一個身分證明附件。
- **FR-004**: 系統必須提供拍照上傳功能,方便使用者直接用手機拍攝並上傳身分證明文件。
- **FR-005**: 系統必須在身分證預覽旁提供 ⭐ AI 辨識按鈕,點擊後自動辨識身分證上的姓名與身分證字號,並以身分證字號搜尋既有客戶。
...
- **FR-009**: 系統必須自動產生唯一的服務單編號,格式為 `BS` + `YYYYMMDD` + 三位流水號（如 BS20251214001）。
...
- **FR-020**: 系統必須自動產生唯一的服務單編號,格式為 `CS` + `YYYYMMDD` + 三位流水號（如 CS20251214001）。
...
- **FR-030**: 詳細頁面必須提供編輯功能,店員可修改服務單內容（商品項目、總金額、寄賣日期、續約設定等）並儲存變更。
- **FR-031**: 系統必須記錄所有服務單修改歷史（變更欄位、舊值、新值、變更時間、變更者）,並在詳細頁面提供「修改歷史」標籤供查詢。
- **FR-032**: 系統必須提供服務單狀態更新功能,狀態選項包含：待處理（PENDING）、已完成（COMPLETED）、已終止（TERMINATED）,狀態轉換規則為終態（TERMINATED）不可逆。
- **FR-033**: 系統必須提供附件管理功能,店員可查看、下載已上傳的身分證明文件、合約文件等附件。
...
- **FR-036**: 系統必須使用樂觀鎖（version 欄位）控制並發更新,當多位店員同時修改同一筆服務單時,系統偵測到 version 不一致則返回錯誤訊息並要求重新載入最新資料。
- **FR-037**: 系統必須在每次服務單更新後自動增加 version 欄位（初始值 0,每次更新 +1）。
...
- **FR-039**: 系統必須提供簽名合併預覽功能,將客戶的觸控簽名（Base64 PNG）與 PDF 合約文件合併,產生預覽文件供店員與客戶確認,確認無誤後再正式儲存簽名記錄。

#### OCR 辨識規則

- **FR-040**: 系統必須支援辨識台灣身分證（1 英文字母 + 9 數字）與外籍人士居留證（若無統一證號則使用西元出生年月日 8 位數 + 英文姓名前兩字母大寫,總長度 10 位）。
- **FR-041**: OCR 辨識信心度低於 0.8 時,系統必須顯示警告訊息提示店員確認辨識結果。
...
- **Attachment（附件）**: 儲存服務單相關文件（身分證明、合約文件等）,包含檔案名稱、檔案類型、檔案 URL、檔案大小、上傳時間等屬性。
- **SignatureRecord（簽名記錄）**: 記錄線下或線上簽名的完整資訊,包含簽名文件類型、簽名方式、簽名資料（Base64 PNG）或 Dropbox Sign 請求 ID、簽名者姓名、簽名時間等屬性。
- **ModificationHistory（修改歷史）**: 追蹤服務單的所有變更,包含變更欄位、舊值、新值、變更時間、變更者等屬性,提供稽核與回溯能力。
