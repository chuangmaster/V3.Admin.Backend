...
public class LoginDto { }                  // ❌ 錯誤 (使用 Request 而非 Dto)
...
- Service 層接收並轉換為 Entity

---

#### 1.3 Response (回應 DTO)

**定義**: API 層的輸出模型,用於回傳資料給客戶端。
...
│   ├── Role.cs
│   └── Permission.cs
├── Requests/              # Request DTO 類別
│   ├── LoginRequest.cs
│   ├── CreateAccountRequest.cs
│   └── UpdateAccountRequest.cs
├── Responses/             # Response DTO 類別
│   ├── LoginResponse.cs
│   ├── AccountResponse.cs
...
| 層級 | 使用模型 | 說明 |
|-----|---------|------|
| **Controller** | Request, Response | 接收 Request,回傳 Response (包裝於 ApiResponseModel) |
| **Service** | Request, Response, Entity, View | 轉換 Request → Entity,Entity/View → Response |
...
| 類型 | 命名規則 | 範例 |
|-----|---------|------|
| **Entity** | PascalCase, 名詞單數 | `User`, `Role` |
| **Request DTO** | PascalCase, 動詞/名詞 + `Request` | `LoginRequest`, `CreateAccountRequest` |
| **Response DTO** | PascalCase, 名詞 + `Response` | `AccountResponse`, `LoginResponse` |
| **View DTO** | PascalCase, 名詞 + `View` | `UserRoleView`, `AccountDetailView` |
| **Interface** | `I` + PascalCase | `IUserRepository`, `IAuthService` |
| **Service** | PascalCase, 名詞 + `Service` | `AuthService`, `AccountService` |
| **Repository** | PascalCase, 名詞 + `Repository` | `UserRepository` |
| **Controller** | PascalCase, 名詞 + `Controller` | `AuthController` |
| **資料表** | snake_case, 複數 | `users`, `user_roles` |
| **欄位** | snake_case | `username`, `created_at` |
| **索引** | `idx_` + snake_case | `idx_users_username` |
